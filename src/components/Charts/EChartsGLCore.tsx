...
import { EChartsReactProps, EChartsInstance } from 'echarts-for-react';


/**
 * core component for echarts binding
 */
export default class EChartsGLCore extends PureComponent<EChartsReactProps> {
  /**
   * echarts render container
   */
  public ele: HTMLElement | HTMLDivElement;
  ...
  componentDidUpdate(prevProps: EChartsReactProps) {
    ...
    if (isFunction(shouldSetOption) && shouldSetOption && !shouldSetOption(prevProps, this.props)) {
      return;
    }
  ...
  private async renderNewEcharts() {
    ...
    // 4. on chart ready
    if (isFunction(onChartReady) && onChartReady) onChartReady(echartsInstance);
  ...
  // bind the events
  private bindEvents(instance: { on: (arg0: string, arg1: (param: any) => void) => void; }, events: EChartsReactProps['onEvents']) {
  ...
  render(): JSX.Element {
    ...
    return (
      <div
        ref={(e: HTMLDivElement) => this.ele = e as HTMLElement}
        style={newStyle}
